...
system.time(zzz <- mcmc5.abc(nits1)) # TODO: TEST mcmc5 | mcmc3a
zzz$acp/nits1
...
# SAVE image
save.image(file="data/om5a/image_abc5a.rda", compress="xz")
...
# SAVE runs
save(mczzz, file="data/om5a/mcmc_abc5a.rda", compress="xz")

# EXTRACT
load('data/om5a/mcmc_abc5a.rda')
load('data/om5a/image_abc5a.rda')
...
# SAVE output
save(mcpars, mcvars, C, file="data/om5a/mcvars_abc5a.rda", compress="xz")
...
load('data/base.rda')
load('data/om5a/mcmc_abc5a.rda')
load('data/om5a/mcvars_abc5a.rda')
...
save(out, file="data/om5a/mcout_abc5a.rda", compress="xz")
...
swi <- Reduce(join, lapply(base$ids[9:12], index))

SW <- FLIndexBiomass(index=propagate(swi[, ac(2000:2020)], its),
...
# stk: no units
oem <- FLoem(observations=list(ALB=list(idx=FLIndices(SW=SW),
...
save(om, oem, file='data/om5a/om5a-raw.rda', compress='xz')

# }}}

# --- EXTEND {{{

load('data/om5a/om5a-raw.rda')
load('data/om5a/mcout_abc5a.rda')
...
# SAVE
save(om, oem, file='data/om5a/om5a_extended.rda', compress='xz')

# }}}

# -- UPDATE for new NC {{{

load('data/om5a/om5a_extended.rda')
...
# SAVE
save(om, oem, file="data/om5a/om5a_updated.rda", compress="xz")
...
# LOAD om
load('data/om5a/om5a_updated.rda')
...
# SAVE as qs2 (faster)
qs_savem(om, oem, file='data/om5a.qs2')
...
qs_readm(om, oem, file='model/om5a.qs2')
...
save(fom, file="data/om5a/om5a_fwd.rda", compress="xz")
