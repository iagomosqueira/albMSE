...
1948 # min year for sdreport outputs (-1 for styr); #_1948
-2 # max year for sdreport outputs (-1 for endyr+1; -2 for endyr+Nforecastyrs); #_2030
...
4 # F_std_reporting_units: 0=skip; 1=exploitation(Bio); 2=exploitation(Num); 3=sum(Apical_F's); 4=mean F for range of ages (numbers weighted); 5=unweighted mean F for range of ages
 1 12 # min and max age over which mean F will be calculated, with F=Z-M
2 # F_std_scaling: 0=no scaling; 1=F/Fspr; 2=F/Fmsy; 3=F/Fbtgt; where F means annual F_std, Fmsy means F_std@msy; values >=11 invoke N multiyr using 10s and 100s digit; append .1 to invoke log(ratio)
...
-1 # random number seed for bootstrap data (-1 to use long(time) as seed): # 1750365141
